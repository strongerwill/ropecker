...
/* Define the following interface functions only for outer projects. */
	 ...
	 */
		...
		extern _DecodeResult distorm_decompose32(_CodeInfo* ci, _DInst result[], unsigned int maxInstructions, unsigned int* usedInstructionsCount);
		#define distorm_decompose distorm_decompose32

		#ifndef DISTORM_LIGHT
			/* If distorm-light is defined, we won't export these text-formatting functionality. */
			extern _DecodeResult distorm_decode32(_OffsetType codeOffset, const unsigned char* code, int codeLen, _DecodeType dt, _DecodedInst result[], unsigned int maxInstructions, unsigned int* usedInstructionsCount);
			extern void distorm_format32(const _CodeInfo* ci, const _DInst* di, _DecodedInst* result);
 ...
 */
extern unsigned int distorm_version(void);
